...
import Nav from "../../Nav";
import { PictureInfo, apiGetCurrentSessionUser, apiPictureLike, apiPictureSearch, apiPictureUnlike } from "../../dao";
...
function SearchEntry({ pictureInfo, userData }: { pictureInfo: PictureInfo, userData: User | false }) {
  ...
  function LikeButton() {
    return (
      <div>
        {
          userData && (
            <div onMouseEnter={() => setHoveringOver(false)} onMouseLeave={() => setHoveringOver(true)}>
              <button onClick={e => onLike(e)} style={{ border: 'none', background: 'none', cursor: 'pointer' }}>
  ...
  return (
    ...
    >
      <div style={{display: 'flex', alignItems: 'center', padding: '2%'}}>Image #{pictureInfo.id}</div>
      <img src={pictureInfo.previewURL} style={{objectFit: 'fill', height: '100%'}}/>
    ...
    <div>
      ...
      <div style={{marginLeft: '5%', marginRight: '5%', position: 'relative'}}>
        ...
        <div style={{display: 'flex', alignItems: 'center', flexDirection: 'column'}}>
          {imageEntries.slice(10).map((imageEntry, i) => <SearchEntry pictureInfo={imageEntry} userData={userData} key={i}/>)}
        </div>
      </div>
    </div>
  );
